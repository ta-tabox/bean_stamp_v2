...
test("ロースター所属ユーザーは Offer を作成・更新・削除でき、wanted users 画面に到達できる", async ({
  ...
  await expect(page.getByText("焙煎日はオファー終了日より後の日付を指定してください")).toBeVisible()
  await expect(page.getByTestId("offer-mutation-form")).toHaveAttribute("data-form-ready", "1")
  ...
  await page.getByRole("link", { name: "Wanted users" }).click()
  await expect(page).toHaveURL(/\/offers\/\d+\/wanted_users$/)
  await expect(page.getByText("Wanted users はまだいません")).toBeVisible()

  await page.getByRole("link", { name: "オファー詳細" }).click()
  ...
  await page.getByRole("button", { name: "オファーを削除する" }).click()
