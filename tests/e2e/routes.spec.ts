...
test("ロースター所属ユーザーは Offer を作成・更新・削除でき、wanted users 画面に到達できる", async ({
  ...
  await page.goto("/roasters/home")
  await expect(page.getByRole("heading", { name: "Offer Roasterのホーム", exact: true })).toBeVisible()
  ...
  await expect(page.getByText("酸味").first()).toBeVisible()
  await expect(page.getByText("フレーバー").first()).toBeVisible()
